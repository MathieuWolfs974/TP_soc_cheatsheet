...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const critAlertTx = document.getElementById('critAlertText');
    ...
    Array.from(tbody.querySelectorAll('tr:not(.events__empty)')).forEach(function (tr) {
        ...
        events.push({
            ...
            type:   (c[3].dataset.type   || c[3].textContent).trim(),
            action: (c[4].dataset.action || c[4].textContent).trim(),
    ...
    function tagHtml(action) {
        ...
        const ico = action === 'Bloqué' ? '✕' : '✓';   // icône + texte + couleur
        return '<span class="tag ' + cls + '"><span aria-hidden="true">' + ico + '</span> ' + action + '</span>';
    }

    // Pastille de criticité : FORME distincte + texte + couleur (accessibilité)
    const CRIT_META = {
        critique: ['◆', 'Critique'],
        warning:  ['▲', 'Warning'],
        info:     ['●', 'Info']
    ...
    function critChip(crit) {
        const m = CRIT_META[crit] || CRIT_META.info;
        return '<span class="crit crit--' + crit + '"><span class="crit__ico" aria-hidden="true">'
             + m[0] + '</span> ' + m[1] + '</span>';
    }
    ...
    function render() {
        ...
        filtered.forEach(function (e) {
            ...
            html += '<tr data-criticite="' + e.crit + '" class="evrow' + flag + '">'
                  ...
                  + '<td data-label="' + COLS[3] + '">' + critChip(e.crit) + e.type + '</td>'
    ...
    function tick() {
        ...
        // SPEC 2 — un nouvel événement CRITIQUE alimente la région role="alert",
        // ce qui déclenche une annonce immédiate par le lecteur d'écran.
        if (e.crit === 'critique') {
            critAlertTx.textContent = 'Nouvelle alerte critique — ' + e.type + ' depuis ' + e.ip + ' à ' + e.time + '.';
            critAlert.hidden = false;
        }
